...
const ChatInterface: React.FC<Props> = ({ text, language, onBack, userId }) => {
  ...
  useEffect(() => {
    ...
    chatSession.current = createAgriChatSession(langName, geminiHistory);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        result = await chatSession.current.sendMessage({ message: parts as any }); 
      } else {
        result = await chatSession.current.sendMessage({ message: currentInput });
      }

      const responseText = result.text || "I'm having trouble connecting to the field network. Please try again.";
